...
function deObjetoAarray(objeto) {
  ...
  for (let clave in objeto) {
    ...
    let nuevoArreglo = [clave, valor];
    arregloPadre.push(nuevoArreglo)
  }
  return arregloPadre;
}
...
function capicua(numero) {
  ...
  let numReverseString = numString.split('').reverse().join('');
  let stringNum = parseInt(numReverseString);
...
function deleteAbc(string) {
  ...
  for (let char in string) {
    if (string[char] !== "a" && string[char] !== "b" && string[char] !== "c") {
      nuevoString += string[char];
    }
  }
  return nuevoString;
}
